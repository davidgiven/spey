...
static void initialise(SpeyCLI& cli)
{
	Threadlet::initialise();
	Sql.open(cli.d());
	ServerProcessor::FromAddress.set(cli.f());
	ServerProcessor::ToAddress.set(cli.t());
...
static int daemonmode(SpeyCLI& cli)
{
...
static int inetdmode(SpeyCLI& cli)
{
...
int main_spey(int argc, char* argv[])
{
	SpeyCLI cli(argc, argv);
